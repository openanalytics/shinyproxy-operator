...
class ConfigMapCreator(private val shinyProxy: ShinyProxy) {

    private val logger = KotlinLogging.logger {}

    private val configMapDefinition: ConfigMap = ConfigMapBuilder()
            .withNewMetadata()
            .withGenerateName(shinyProxy.metadata.name.toString() + "-configmap-")
                ...
                .endOwnerReference()
            .endMetadata()
            .addToData("application-in.yml", shinyProxy.getSpec().applicationYaml)
            .build()

    fun create(kubeClient: KubernetesClient): ConfigMap {
